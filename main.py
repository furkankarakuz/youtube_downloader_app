    ...
    def clicked_start_button(self):
        try:
            if self.video_check.isChecked() or self.mp3_check.isChecked():
                ytp = YotubeProcess(r"%s"%(self.dictionary_label.text()),self.url_line.text())
                ytp.download_video()
                if self.mp3_check.isChecked():
                    ytp.convert_mp3(not(self.video_check.isChecked()))
        except:
            QMessageBox.warning(self,"Error","error occurred during operation.Please , try it")
    
    def clicked_dictionary_button(self):
        selected_dictionary = QFileDialog.getExistingDirectory(self,app.applicationName()+" - Open File",self.dictionary_label.text())
